...
ai-core v{VERSION} - We preserve the corpus of human knowledge - we do not rewrite it in our own image.
...
async def lifespan(app: FastAPI):
    ...
    if app.state.redis is not None:
        redis_task = asyncio.create_task(redis_task_command_listener(app))
        app.state.redis_task_command_listener = redis_task
        app.state.background_tasks.append(redis_task)

    if THREAD_POOL_SIZE and THREAD_POOL_SIZE > 0:
        limiter = anyio.to_thread.current_default_thread_limiter()
        limiter.total_tokens = THREAD_POOL_SIZE

    usage_task = asyncio.create_task(periodic_usage_pool_cleanup())
    app.state.background_tasks.append(usage_task)
    ...
    if app.state.config.ENABLE_BASE_MODELS_CACHE:
        try:
            ...
            log.warning(f"Failed to load base models on startup: {e}")

    yield

    # Properly cancel all background tasks
...
# Check if we should skip model loading entirely
SKIP_MODEL_LOADING = os.environ.get("SKIP_MODEL_LOADING", "false").lower() == "true"

def load_models_with_timeout():
    """Load models with a 30-second timeout"""
    try:
        log.info("Loading embedding and reranking models...")
        ...
        try:
            ...
            log.info("Reranking model loaded successfully")
        except Exception as e:
            log.error(f"Failed to load reranking model: {e}")
            app.state.rf = None
            
    except Exception as e:
        log.error(f"Error loading models: {e}")

# Load models with timeout (or skip entirely)
if SKIP_MODEL_LOADING:
    log.info("SKIP_MODEL_LOADING is set to true - skipping model loading")
    app.state.ef = None
    app.state.rf = None
else:
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(load_models_with_timeout)
            future.result(timeout=120)  # 120 second timeout (increased from 30)
    except TimeoutError:
        log.error("Model loading timed out after 120 seconds - continuing without models")
        app.state.ef = None
        app.state.rf = None
    except Exception as e:
...
try:
    ...
    log.info("Embedding function initialized successfully")
except Exception as e:
    log.error(f"Failed to initialize embedding function: {e}")
    app.state.EMBEDDING_FUNCTION = None

# Initialize reranking function with fallback
try:
    ...
    log.info("Reranking function initialized successfully")
except Exception as e:
    log.error(f"Failed to initialize reranking function: {e}")
    app.state.RERANKING_FUNCTION = None
...
def log_startup():
    # Only log via the logger, not print or file
    log.info(f"[STARTUP] Open WebUI FastAPI app starting at {time.strftime('%Y-%m-%d %H:%M:%S')} | PID: {os.getpid()} | ENV: {os.environ.get('ENV', 'unset')} | HOST: {os.environ.get('HOST', 'unset')} | PORT: {os.environ.get('PORT', 'unset')}")
    embedding_status = "loaded" if app.state.ef is not None else "failed/not loaded"
    reranking_status = "loaded" if app.state.rf is not None else "failed/not loaded"
    embedding_func_status = "initialized" if app.state.EMBEDDING_FUNCTION is not None else "failed/not initialized"
    reranking_func_status = "initialized" if app.state.RERANKING_FUNCTION is not None else "failed/not initialized"
    log.info(f"[STARTUP] Model Status - Embedding Model: {embedding_status}, Reranking Model: {reranking_status}, Embedding Function: {embedding_func_status}, Reranking Function: {reranking_func_status}")
    log.info(f"[STARTUP] Application is ready and listening on port {os.environ.get('PORT', '8080')}")

@app.exception_handler(Exception)
async def global_exception_handler(request, exc):
    log.error(f"[UNHANDLED EXCEPTION] {repr(exc)} at {time.strftime('%Y-%m-%d %H:%M:%S')}")
