...
async def update_password(
    ...
    # Disable password updates when Auth0 is enabled
    if ENABLE_AUTH0:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Password updates are disabled when Auth0 is enabled. Please use Auth0 for password management.",
        )
...
async def signin(request: Request, response: Response, form_data: SigninForm):
    # Disable local authentication when Auth0 is enabled
    if ENABLE_AUTH0:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Local authentication is disabled when Auth0 is enabled. Please use Auth0 for authentication.",
        )
...
async def signup(request: Request, response: Response, form_data: SignupForm):
    # Disable local registration when Auth0 is enabled
    if ENABLE_AUTH0:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Local registration is disabled when Auth0 is enabled. Please use Auth0 for user registration.",
        )
...
async def add_user(form_data: AddUserForm, user=Depends(get_admin_user)):
    # Disable local user creation when Auth0 is enabled
    if ENABLE_AUTH0:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Local user creation is disabled when Auth0 is enabled. Please use Auth0 for user management.",
        )
