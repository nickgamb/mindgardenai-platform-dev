...
# Import Auth0 authentication if enabled
if ENABLE_AUTH0:
    from open_webui.utils.auth0_auth import get_auth0_user_from_token
...
def get_current_user(
    ...
    if ENABLE_AUTH0:
        try:
            ...
            user = get_auth0_user_from_token(token, request, frontend_email)
            if user:
                # Add user info to current span
                current_span = trace.get_current_span()
                if current_span:
                    ...
                    current_span.set_attribute("client.user.email", user.email)
                    current_span.set_attribute("client.user.role", user.role)
                    current_span.set_attribute("client.auth.type", "auth0")
            ...
            log.error(f"Auth0 authentication error: {e}")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Auth0 authentication failed",
            )

    # auth by jwt token (fallback for non-Auth0 mode)
    try:
...
def get_verified_user(request: Request):
    ...
    if ENABLE_AUTH0:
        if not token:
            raise HTTPException(status_code=401, detail="Not authenticated (Auth0)")
        # Extract email from frontend if provided via custom header
        frontend_email = request.headers.get('X-User-Email')
        user = get_auth0_user_from_token(token, request, frontend_email)
        if not user:
        ...
        return user
    # Fallback to existing session-based logic
    user = None
    if hasattr(request.state, 'user') and request.state.user:
        user = request.state.user
    elif hasattr(request, 'session') and request.session.get('user'):
