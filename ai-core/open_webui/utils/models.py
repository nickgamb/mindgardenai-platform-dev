...
async def fetch_ollama_models(request: Request, user: UserModel = None):
    log.debug("fetch_ollama_models called")
    raw_ollama_models = await ollama.get_all_models(request, user=user)
    log.debug(f"fetch_ollama_models: raw_ollama_models keys={list(raw_ollama_models.keys()) if raw_ollama_models else None}")
    return [
        {
            "id": model["model"],
            "name": model["name"],
        ...
        for model in raw_ollama_models.get("models", [])
    ]
...
async def get_all_base_models(request: Request, user: UserModel = None):
    log.debug(f"get_all_base_models: ENABLE_OLLAMA_API={getattr(request.app.state.config, 'ENABLE_OLLAMA_API', None)} ENABLE_OPENAI_API={getattr(request.app.state.config, 'ENABLE_OPENAI_API', None)}")
    openai_task = (
        fetch_openai_models(request, user)
        if getattr(request.app.state.config, 'ENABLE_OPENAI_API', False)
        else asyncio.sleep(0, result=[])
    )
    ollama_task = (
        fetch_ollama_models(request, user)
        if getattr(request.app.state.config, 'ENABLE_OLLAMA_API', False)
        else asyncio.sleep(0, result=[])
    )
    ...
    log.debug(f"get_all_base_models: openai_models={len(openai_models)}, ollama_models={len(ollama_models)}, function_models={len(function_models)}")
